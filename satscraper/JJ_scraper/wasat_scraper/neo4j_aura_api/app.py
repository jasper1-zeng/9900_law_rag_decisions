#!/usr/bin/env python3
"""
neo4j_aura_api.py - Neo4j Aura API for the WASAT case database.

This script provides a REST API for the Neo4j Aura database containing WASAT case data.
It also includes the web interface for visualizing the case network.
"""
...
@ns_network.route('/citations')
class NetworkVisualizationGet(Resource):
    @api.doc(description="Get a network of cases and their relationships using a GET request",
             params={'citations': 'Comma-separated list of citation numbers',
                     'max_depth': 'Maximum relationship depth (default: 1, max: 3)'})
    @api.response(200, 'Success', network_model)
    @api.response(400, 'Bad Request')
    @api.response(500, 'Server Error')
    def get(self):
        """Get a network of cases and their relationships using a GET request."""
        try:
            # Get the comma-separated list of citations from the URL
            citations_param = request.args.get('citations', '')
            if not citations_param:
                logger.error("No citation numbers provided in request")
                return {"error": "No citation numbers provided"}, 400
                
            # Split the citations by comma
            citation_numbers = [c.strip() for c in citations_param.split(',') if c.strip()]
            logger.info(f"Processing citations: {citation_numbers}")
            
            # Validate we have at least one citation
            if not citation_numbers:
                logger.error("No valid citation numbers after splitting and stripping")
                return {"error": "No valid citation numbers provided"}, 400
                
            # Get the max_depth parameter
            try:
                max_depth = min(int(request.args.get('max_depth', 1)), 3)
            except ValueError:
                logger.warning(f"Invalid max_depth parameter: {request.args.get('max_depth')}, defaulting to 1")
                max_depth = 1
                
            logger.info(f"Calling get_case_network with citations={citation_numbers}, max_depth={max_depth}")
            
            # Use the existing method to get the network
            result = neo4j_driver.get_case_network(citation_numbers, max_depth)
            
            if "error" in result:
                logger.error(f"Error returned from get_case_network: {result['error']}")
                return {"error": result["error"]}, 500
            
            # Log success information
            logger.info(f"Successfully retrieved network with {len(result.get('nodes', []))} nodes and {len(result.get('links', []))} links")
            ...
            logger.exception(f"Unhandled exception in /api/network/citations: {str(e)}")
            return {"error": f"Internal server error: {str(e)}"}, 500

@ns_cases.route('/multiple/<path:citation_list>/graph')
class MultiCaseGraphVisualization(Resource):
    @api.doc(description="Get visualization data for multiple cases and their relationships",
             params={'citation_list': 'Comma-separated list of case citation numbers',
                     'max_depth': 'Maximum relationship depth (default: 1, max: 3)'})
    @api.response(200, 'Success', network_model)
    @api.response(404, 'Case not found')
    def get(self, citation_list):
        """Get visualization data for multiple cases and their relationships."""
        # URL decode and split the citation list
        citation_numbers = [c.strip() for c in citation_list.split(',') if c.strip()]
...
def visualizer():
    ...
    # Check if citations parameter is provided
    citations = request.args.get('citations', '')
    
    # Handle citations parameter if provided
    if citations and not data_url:
        # Format citations as a dataUrl for the /api/network/citations endpoint
        data_url = f"/api/network/citations?citations={citations}"
        logger.info(f"Converting citations parameter to dataUrl: {data_url}")
    
    # Check if hideHeader parameter is true
    hide_header = request.args.get('hideHeader', 'false').lower() == 'true'
    
    # Inject the base URL into the template for API calls
    base_url = request.url_root.rstrip('/')
    
    # Log the parameters for debugging
    logger.info(f"Rendering visualizer with base_url={base_url}, data_url={data_url}, citations={citations}, hide_header={hide_header}")
